...
Redoing the analysis taking into account that 

## Setting directory
```{r set dir, include=FALSE}
setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano")
getwd()
```
...
tab <-read_excel("sav_reads_notrans.xlsx", col_names=TRUE) # table containing as first column the MO and the others the different samples
tab_df <- as.data.frame(tab) # convert the table in dataframe
...
df_new <- data.frame(apply(new, 2, function(x) as.numeric(as.character(x)))) # convert the cells of the dataframe in numeric type
...
rows_to_keep <- c("SWT0R1", "SWT0R2", "SWT0R3", 
                "SWT1R1", "SWT1R2", "SWT1R3",
                "SWT2R1", "SWT2R2", "SWT2R3",
                "SWT3R1", "SWT3R2", "SWT3R3",
                "SWT5R2", "SWT5R3") # subset the total table

df_new <- df_new[rownames(df_new) %in% rows_to_keep, ] # take only wine samples
original_colnames <- colnames(df_new) # we have to store the colnames
# because the next function converts the conames with a point where there is a space
df_new <- data.frame(apply(df_new, 2, function(x) as.numeric(as.character(x))))
colnames(df_new) <- original_colnames

# important to use the count table, not the relative abundance table
...
ggplot(data.frame(prevalence = taxa_prevalence), aes(x = prevalence)) +
  geom_histogram(binwidth = 1, fill = "#69b3a2", color = "black") +
  labs(title = "Taxon Prevalence", x = "Number of Samples", y = "Number of Taxa") +
  theme_minimal()

# plot abundance histogram, number of reads found in each microorganism in the all the samples
ggplot(data.frame(log_abundance = log10(taxa_abundance + 1)), aes(x = log_abundance)) +
...
Here better to put the values of the distribution for example mean, median, mode values in a table

```{r Filtering, echo=FALSE}
# 2. Remove samples (rows) with low total counts
# Filter out samples with total count < 1,000
filt_tab <- df_new[rowSums(df_new) >= 1000, ] # take only the rows that have more or equal than 1000 reads
init <- dim(df_new)
fin <- dim(filt_tab)
...
rownames(filt_tab) <- c("SWT0R1", "SWT0R2", "SWT0R3", 
                "SWT1R1", "SWT1R2", "SWT1R3",
                "SWT2R1", "SWT2R2", "SWT2R3",
                "SWT3R1", "SWT3R2", "SWT3R3",
                "SWT5R2", "SWT5R3")
filt_tab$Sample_Group <- sub("R[0-9]+", "", rownames(filt_tab)) # add the column Sample_group column in the table, that have the information of the sample
...
Save this table in an another excel file because it is useful when we will compute the correlation between pathways and metabolites
...
setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/9_Corr_Pathways_MO")
library(writexl)
filt_tab$RowNames <- rownames(filt_tab)
write_xlsx(filt_tab, "table_MO.xlsx")
dim(filt_tab)
```
...
filt_tab[] <- lapply(filt_tab, function(x) as.numeric(as.character(x)))
sample_depths <- rowSums(filt_tab, na.rm=TRUE) # for each sample sum all the reads 
...
ggplot(df, aes(x = "", y = depth)) +  # creation of a graph containing the depth as y
  geom_boxplot() +
...
# cv is a statistic used to describe how variable a set of numbers is relative to their mean
cat("Mean:", mean_depth, "\nMedian:", median_depth, "\nRange:", range_depth, "\nCV:", cv_depth, "\n")
```
...
# since the samples are not balanced, before we have to rarefy the table 
# to compute the alpha diversity, but is possible to use the count table as it is
# (as we are going to do below),
# for beta diversity is suggested to simply normalize and not rarefy the table
metadata_w <- data.frame(
  sample_id = c("SWT0R1", "SWT0R2", "SWT0R3", 
                "SWT1R1", "SWT1R2", "SWT1R3",
                "SWT2R1", "SWT2R2", "SWT2R3",
                "SWT3R1", "SWT3R2", "SWT3R3",
                "SWT5R2", "SWT5R3"),
  timepoint = c("T0", "T0", "T0", 
                ...
                "T3", "T3", "T3", 
                "T5", "T5")
  # assign the time point to the specific sample
)
...
```
? better to explain why we used this type of test (kruskal-wallis) and not other

Rewise the database in input of the alpha diversity and the microbial richness
...
It could be noted that at Time T0 in the richness and Observed alpha diversity plot, the height of the boxplots follows the same pattern, as expected. So from time T0 to T5 the richness in species is initially low at time T0, reaches the peak at time T1 and decreases from time T1 to time T5; in particular at time T5 we have the lowest amount of richness.

In the Shannon and Simpson diversity the pattern is different in particular at time T0, respect to the graphs previously described. Because in TO we have the highest point and after that there is a decrease, reaching the lowest point in T5. It could be explained with the fact that although in T0 the richness is the highest, the evenness lowers the value of the boxplot (so the individuals are distributed in a way that is not even among those species)
...
TukeyHSD(anova_res)
```
Explain why you use this test and not others

The null values are not in all the column of the row but only in some columns
...
tab_taxa <- read_excel("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/1_taxonomy/taxonomy_bracken.xlsx")
...
df_filtered <- df %>% filter(family %in% keep_taxa) # keep only Saccharomycetaceae family, because now we are looking at Saccharomyces yeast
...
png("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/2_Relative_abundance/Saccharomycetac_abundance.png", width = 2000, height = 1500, res = 300)  # 300 dpi is print-quality
...
png("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/2_Relative_abundance/Saccharomyces_abundance.png", width = 2000, height = 1500, res = 300)  # 300 dpi is print-quality
dev.off()
plot(abund_sacch)
```

If we look into microbial dynamics during fermentation, in wine, there is a very high amount of Saccharomycetaceae that are present in high amount and increase their abundance, and reach a steady state at the final time T5. 
To the Saccharomycetaceae belong Saccharomyces cerevisiae and other MO.
...
final_taxa <- read_excel("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/1_taxonomy/taxonomy_bracken.xlsx")
...
# 4. Prune Low-Abundance taxa
# Convert to data frame to long format for filtering
df <- psmelt(ps_relabund)
...
# now we keep all the families, including the one of Saccharomices cerevisiae (Saccharomycetaceae)
...
# we remove this families since t
mold_families <- c(
  "Sclerotiniaceae",
  "Mycosphaerellaceae",
  "Aspergillaceae",
...
png("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/2_Relative_abundance/Yeast_non_saccharomyces.png", width = 1200, height = 900, res = 200)  # 300 dpi is print-quality
plot(abund_yeast)
dev.off()
plot(abund_yeast)
```
...
final_taxa <- read_excel("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/1_taxonomy/taxonomy_bracken.xlsx")
otu_taxa   <- colnames(sub_w)
taxa_filt  <- final_taxa[otu_taxa, , drop = FALSE]
...
# 10. Save plot
png("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/2_Relative_abundance/Yeast_non_saccharomyces_genus_wine.png",
...
plot(abund_yeast_genus)
```
yeast non-sacch - non oenological
```{r Relative abundance by genus, echo=FALSE}
library(phyloseq)
library(dplyr)
library(ggplot2)
...
# 10. Save plot
png("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/2_Relative_abundance/Yeast_non_saccharomyces_genus_no_oeno.png",
    width = 1200, height = 900, res = 200)
plot(abund_yeast_genus)
dev.off()
...
png("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/2_Relative_abundance/Bacteria_all_family.png",
    width = 1200, height = 900, res = 200)
ggplot(df_bact_summary,
       aes(x = timepoint,
           y = mean_abundance,
...
# 6. Force-keep wine-relevant families
wine_relevant <- c(
...
png("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/2_Relative_abundance/Bacteria_genus.png",
    width = 1200, height = 900, res = 200)
plot(abund_yeast_genus)
dev.off()
```
...
# since most beta diversity metrics (like Bray-Curtis, UniFrac) are sensitive to
# sequencing depth. Consider:
# - rarefaction (downsample to even depth) -> common in Bray-Curtis or Jaccard
# - normalization (relative abundance or centered log-ratio) -> if using Aitchison

# filter the table
#### 1. beta diversity in Wine at different timings ####
# we will use the variable metadata_w and the table sub_w
# we use rarefaction
sampleAbund <- rowSums(sub_w) # gives the number of individuals found in each sample
...
# look for each time point if there is significant diversity between the replicates
#metadata_T0 <- subset(metadata_w, timepoint == "T0")
#bray_T0 <- beta_div_bray[rownames(metadata_T0), rownames(metadata_T0)]

#metadata_T1 <- subset(metadata_w, timepoint == "T1")
#bray_T1 <- beta_div_bray[rownames(metadata_T1), rownames(metadata_T1)]

#metadata_T2 <- subset(metadata_w, timepoint == "T2")
#bray_T2 <- beta_div_bray[rownames(metadata_T2), rownames(metadata_T2)]

#metadata_T3 <- subset(metadata_w, timepoint == "T3")
#bray_T3 <- beta_div_bray[rownames(metadata_T3), rownames(metadata_T3)]

#metadata_T5 <-subset(metadata_w, timepoint == "T5")
#bray_T5 <- beta_div_bray[rownames(metadata_T5), rownames(metadata_T5)]
...
#adonis_res <- adonis2(bray_T0 ~ timepoint, data = metadata_w)
#adonis_res <- adonis2(bray_T1 ~ timepoint, data = metadata_w)
#adonis_res <- adonis2(bray_T2 ~ timepoint, data = metadata_w)
#adonis_res <- adonis2(bray_T3 ~ timepoint, data = metadata_w)
#adonis_res <- adonis2(bray_T5 ~ timepoint, data = metadata_w)
```
...
# we will use the variable metadata_w and the table sub_w
# we use rarefaction
sampleAbund <- rowSums(sub_w_filtered) # gives the number of individuals found in each sample
# and we will take the number of the sample with less species
# rarefy to the minimum sample depth
min_depth <- min(sampleAbund) # check the minimum of the sample abundance
...
write.csv(pcoa_df, "pcoa_points_wine.csv", row.names=TRUE)
```
...
rownames(sub_w_filtered) <- c("SWT0R1", "SWT0R2", "SWT0R3", "SWT1R1", "SWT1R2", "SWT1R3", "SWT2R1", "SWT2R2", "SWT2R3", "SWT3R1", "SWT3R2", "SWT3R3", "SWT5R2", "SWT5R3")
...
png("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/3_NMDS/nmds_nobact.png", width = 2000, height = 1500, res = 300)  # 300 dpi is print-quality
ordiplot(nmds, type="n") #Ordination plot function especially for congested plots
orditorp(nmds, display="species", col="red", air=0.01) #The function adds text or points to ordination plots
orditorp(nmds, display="sites", cex=1.25, air=0.01)
dev.off()
...
#rownames(df_w) <- c("SWT0", "SWT1", "SWT2", "SWT3", "SWT5")
...
png("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/3_NMDS/nmds_no_bact_top15.png", width = 2800, height = 1200, res = 200)
ordiplot(nmds, type = "n")
# Add sample points (black)
orditorp(nmds, display = "sites", cex = 1.25, air = 0.01)
# Add only top 15 species (in red)
...
ggplot(nmds_scores, aes(x = NMDS1, y = NMDS2, color = timepoint)) +
  ...
  labs(title = "NMDS of Wine Samples (Bray-Curtis, Hellinger-transformed)",
       x = "NMDS1", y = "NMDS2", color = "Timepoint")
png("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/3_NMDS/nmds_no_bact_top3.png", width = 2800, height = 1200, res = 200)
ordiplot(nmds, type = "n")
# Add sample points (black)
orditorp(nmds, display = "sites", cex = 1.25, air = 0.01)
# Add only top 15 species (in red)
...
microbes <- colnames(sub_w_filtered)
rownames(sub_w_filtered) <- c("SWT0R1", "SWT0R2", "SWT0R3", 
                              "SWT1R1", "SWT1R2", "SWT1R3", 
                              "SWT2R1", "SWT2R2", "SWT2R3", 
                              "SWT3R1", "SWT3R2", "SWT3R3", 
                              "SWT5R2", "SWT5R3")
...
png("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/3_NMDS/nmds_bact.png", width = 2000, height = 1500, res = 300)  # 300 dpi is print-quality
ordiplot(nmds, type="n") #Ordination plot function especially for congested plots
orditorp(nmds, display="species", col="red", air=0.01) #The function adds text or points to ordination plots
orditorp(nmds, display="sites", cex=1.25, air=0.01)
dev.off()
...
There are different tools for differential expression analysis that can be used: 
Kruskal wallis test, DeSEQ, ANCOM.
...
We saw taht the two methods for differential microbial composition give very different results so we decide to use normalizatoin methods before ANCOM (not before DEXSeq, because it has its own normalization methods)

There is uncertainty which type of tool is better to use for differential microbial composition, because there are many tools and is known that they give very different results.

Fot this reason is suggested to give a robust result to use different tools (rewise the use of ANCOM2 because i have not follow the tutorial)
...
filt_tab_lef <- filt_tab[rownames(filt_tab) %in% c("SWT0R1", "SWT0R2", "SWT0R3", "SWT1R1", "SWT1R2", "SWT1R3"),]
...
write_xlsx(tab_lefse, "tab_LEfSe_T0_vs_T1.xlsx")
```
```{r pressure, echo=FALSE}
...
# subset to sample T0 and T1
# take from the dataframe only the T0 and T1
filt_tab_lef <- filt_tab[rownames(filt_tab) %in% c("SWT1R1", "SWT1R2", "SWT1R3", "SWT2R1", "SWT2R2", "SWT2R3"),]
...
# N.B. samples that share the same timepoint value are treated as biological replicates
metadata_w <- data.frame(
  sample_id = c("SWT1R1", "SWT1R2", "SWT1R3", 
                "SWT2R1", "SWT2R2", "SWT2R3"),
...
library(writexl)
setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/")
write_xlsx(tab_lefse, "tab_LEfSe_T1_vs_T2.xlsx")
```
...
# subset to sample T0 and T1
# take from the dataframe only the T0 and T1
filt_tab_lef <- filt_tab[rownames(filt_tab) %in% c("SWT2R1", "SWT2R2", "SWT2R3", "SWT3R1", "SWT3R2", "SWT3R3"),]
otu_names <- sprintf("Otu %03d", 1:697)
colnames(filt_tab_lef) <- otu_names
count_data <- as.matrix(t(filt_tab_lef))
...
# N.B. samples that share the same timepoint value are treated as biological replicates
metadata_w <- data.frame(
  sample_id = c("SWT2R1", "SWT2R2", "SWT2R3", 
                "SWT3R1", "SWT3R2", "SWT3R3"),
...
library(writexl)
setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/")
write_xlsx(tab_lefse, "tab_LEfSe_T2_vs_T3.xlsx")
```
...
# subset to sample T0 and T1
# take from the dataframe only the T0 and T1
filt_tab_lef <- filt_tab[rownames(filt_tab) %in% c("SWT3R1", "SWT3R2", "SWT3R3", "SWT5R2", "SWT5R3"),]
otu_names <- sprintf("Otu %03d", 1:697)
colnames(filt_tab_lef) <- otu_names
count_data <- as.matrix(t(filt_tab_lef))
...
# N.B. samples that share the same timepoint value are treated as biological replicates
metadata_w <- data.frame(
  sample_id = c("SWT3R1", "SWT3R2", "SWT3R3",
                "SWT5R2", "SWT5R3"),
...
fot the time t3 vs t5 i have no markers identified by lefse
```{r pressure, echo=FALSE}
#tab_lefse <- as.data.frame(as.matrix(marker_table(lefse)))
```
...
```{r pressure, echo=FALSE}
library(writexl)
#setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/1_DA_pairwise_w_t/")
#write_xlsx(tab_lefse, "tab_LEfSe_T3_vs_T5.xlsx")
```
...
##### T0 VS T1 ########
## pairwise comparison between timings (TO and T1, T1 and T2, T2 and T3, and T3 and T5)
```{r pressure, echo=FALSE}
# subset to sample T0 and T1
# take from the dataframe only the T0 and T1
sub_ww <- sub_w[rownames(sub_w) %in% c("SWT0R1", "SWT0R2", "SWT0R3", "SWT1R1", "SWT1R2", "SWT1R3"),]
...
setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t")
write.csv(new_sig, "deseqt0vst1.csv", row.names = TRUE)
```


There are microorganisms that have NA values, this are unclassified MO by kraken

#### T1 vs T2
```{r pressure, echo=FALSE}
# subset to sample T0 and T1
# take from the dataframe only the TO and T1
sub_w12 <- sub_w[rownames(sub_w) %in% c("SWT1R1", "SWT1R2", "SWT1R3", "SWT2R1", "SWT2R2", "SWT2R3"), ]
...
new_sig <- sig_taxa[,c(2,6)]
setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t")
write.csv(new_sig, "deseqt1vst2.csv", row.names = TRUE)
```
...
# subset to sample T0 and T1
# take from the dataframe only the TO and T1
sub_w23 <- sub_w[rownames(sub_w) %in% c("SWT2R1", "SWT2R2", "SWT2R3", "SWT3R1", "SWT3R2", "SWT3R3"), ]
...
new_sig <- sig_taxa[,c(2,6)]
setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t")
write.csv(new_sig, "deseqt2vst3.csv", row.names = TRUE)
```
...
# take from the dataframe only the TO and T1
sub_w35 <- sub_w[rownames(sub_w) %in% c("SWT3R1", "SWT3R2", "SWT3R3", "SWT5R2", "SWT5R3"), ]
...
# N.B. samples that share the same timepoint value are treated as biological replicates
metadata_w <- data.frame(
  sample_id = c("SWT3R1", "SWT3R2", "SWT3R3", "SWT5R2", "SWT5R3"),
...
new_sig <- sig_taxa[,c(2,6)]
setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t")
write.csv(new_sig, "deseqt3vst5.csv", row.names = TRUE)
```

## MAASLIN2 ##
MaAaLin2 requires two input files: 
- taxonomic or functional feature abundances: tab-delimeted file, features as columns and samples as rows
...
```{r pressure, echo=FALSE}
setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/maaslin2")
getwd()

# subset to sample T0 and T1
# take from the dataframe only the T0 and T1
filt_tab_lef <- filt_tab[rownames(filt_tab) %in% c("SWT0R1", "SWT0R2", "SWT0R3", 
                                                   "SWT1R1", "SWT1R2", "SWT1R3"),]
...
## T0 vs T1 ##
```{r pressure, echo=FALSE}
setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/maaslin2/T0\ vs\ T1")
...
## T1 vs T2 maaslin 2 ##
```{r pressure, echo=FALSE}
setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/maaslin2/T1 vs T2/")
...
## T2 vs T3 maaslin 2 ##
```{r pressure, echo=FALSE}
setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/maaslin2/T2 vs T3/")
results_23 <- read.delim("all_results.tsv", stringsAsFactors = FALSE)
```
...
# venn diagram
library(readxl)
lefse <- read_excel("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/tab_LEfSe_T0_vs_T1.xlsx")
deseq <- read_csv("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/deseqt0vst1.csv")
maaslin <- read.delim("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/maaslin2/T0 vs T1/all_results.tsv")
...
```{r pressure, echo=FALSE, warning=FALSE}
library(writexl)
setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/ENRICHED/t0vst1/")
getwd()
write_xlsx(lefse_bacteria_t0_t1, "bacteria_t0vst1.xlsx")
write_xlsx(lefse_fungi_t0_t1, "fungi_t0vst1.xlsx")
```
...
library(readxl)
library(readr)
lefse <- read_excel("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/tab_LEfSe_T1_vs_T2.xlsx")
deseq <- read_csv("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/deseqt1vst2.csv")
maaslin <- read.delim("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/maaslin2/T1 vs T2/all_results.tsv")
...
```{r pressure, echo=FALSE, warning=FALSE}
library(writexl)
setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/ENRICHED/t1vst2/")
getwd()
write_xlsx(lefse_bacteria_t1_t2, "bacteria_t1vst2.xlsx")
write_xlsx(lefse_fungi_t1_t2, "fungi_t1vst2.xlsx")
```
...
library(readxl)
library(readr)
lefse <- read_excel("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/tab_LEfSe_T2_vs_T3.xlsx")
deseq <- read_csv("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/deseqt2vst3.csv")
maaslin <- read.delim("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/maaslin2/T2 vs T3/all_results.tsv")
...
```{r pressure, echo=FALSE, warning=FALSE}
library(writexl)
setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/ENRICHED/t2vst3/")
getwd()
write_xlsx(lefse_bacteria_t2_t3, "bacteria_t2vst3.xlsx")
write_xlsx(lefse_fungi_t2_t3, "fungi_t2vst3.xlsx")
```
...
library(readxl)
library(readr)
#lefse <- read_excel("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/tab_LEfSe_T3_vs_T5.xlsx")
deseq <- read_csv("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/deseqt3vst5.csv")
maaslin <- read.delim("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/maaslin2/T3 vs T5/all_results.tsv")
...
Extract from maaslin result which ones have been found enriched in T3 and T5
```{r pressure, echo=FALSE, warning=FALSE}
# first we have to filter from the lefse result, the 119 MO
# Create dataframe
df <- data.frame(
  Microorganism = c(
    ...
    "Fructobacillus americanaquae",
    "Sozzivirus S13",
    "Staphylococcus cohnii",
    "Pseudomonas synxantha",
    "Pseudomonas sp. HN8-3",
    "Bacillus sp. 1NLA3E",
    "Thermobifida alba"
  ),
  log2FoldChange = c(
  ...
  padj = c(
    0.0438531774923058,
    0.00606326720818792,
    2.33566590030906e-05,
    0.000239786333026497,
    2.5543550918148e-05,
    2.33566590030906e-05,
    1.80535860583751e-19,
    0.000554653726492403,
    0.00317204303343861,
    2.42940433527331e-10,
    8.85818013130165e-16,
    0.0068511927122607,
    3.30961769211574e-06
  )
)

# Add enrichment column
df$Enriched_in <- ifelse(df$log2FoldChange > 0, "T5", "T3")

# Select relevant columns
result <- df[, c("Microorganism", "Enriched_in")]
...
```{r pressure, echo=FALSE, warning=FALSE}
# take the taxonomy, merge on species name in common
t3_t5_diff_taxonomy <- tab_taxa_in_otu[tab_taxa_in_otu$Species %in% df$Microorganism,] # take only the rows that have the microorganisms of our interest
# the kingdoms that are present are: Bacteria, Eukaryota and Viruses

t3_t5_diff_taxonomy_eucaria <- t3_t5_diff_taxonomy[t3_t5_diff_taxonomy$Kingdom %in% "Eukaryota", ] 
# take away the rows that have in the column kingdom the ones belonging to the Bacteria, and keep the vector of names in a variable
t3_t5_diff_MO_eu <- t3_t5_diff_taxonomy_eucaria$Species
length(t3_t5_diff_MO_eu)
```
...
t3_t5_diff_taxonomy_bacteria <- t3_t5_diff_taxonomy[t3_t5_diff_taxonomy$Kingdom %in% "Bacteria", ] 
# take away the rows that have in the column kingdom the ones belonging to the Bacteria, and keep the vector of names in a variable
t3_t5_diff_MO_bact <- t3_t5_diff_taxonomy_bacteria$Species
length(t3_t5_diff_MO_bact)
```

# Put this list of bacteria in a table and also create another colum "Enriched in"
```{r pressure, echo=FALSE, warning=FALSE}
# take the 61 MO that are present in the table
sub_df <- df[,c("Microorganism", "Enriched_in")]
sub_df
```
...
```{r pressure, echo=FALSE, warning=FALSE}
library(writexl)
setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/ENRICHED/t3vst5/")
getwd()
write_xlsx(sub_df, "bacteria_t3vst5.xlsx")
```
...
Take away from the significant microorganisms found in the DA analsys the ones that are bacteria
...
write_xlsx(MO_sign_unique, "MO_eu_DA.xlsx")
```
...
```{r pressure, echo=FALSE, warning=FALSE}
setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/4_DA_pairwise_w_t/results_MO_significant")
getwd()
# take the mean values (between replicates of the MO of interest)

# take the filt_tab table with only the sample of wine
sub_tab_filt <- filt_tab[rownames(filt_tab) %in% c("SWT0R1", "SWT0R2", "SWT0R3", "SWT1R1", "SWT1R2", "SWT1R3","SWT2R1","SWT2R2", "SWT2R3", "SWT3R1", "SWT3R2","SWT3R3", "SWT5R2","SWT5R3"), ]
...
for (col in t0_t1_diff_MO) {
       col_mean_t0 = mean(sub_tab_filt[c("SWT0R1", "SWT0R2", "SWT0R3"), col])
       mean_df_t0 <- rbind(mean_df_t0, data.frame(col = col, mean = col_mean_t0))
       col_mean_t1 = mean(sub_tab_filt[c("SWT1R1", "SWT1R2", "SWT1R3"), col])
...
for (col in t1_t2_diff_MO) {
       col_mean_t1 = mean(sub_tab_filt[c("SWT1R1", "SWT1R2", "SWT1R3"), col])
       mean_df_t1 <- rbind(mean_df_t1, data.frame(col = col, mean = col_mean_t1))
       col_mean_t2 = mean(sub_tab_filt[c("SWT2R1", "SWT2R2", "SWT2R3"), col])
...
for (col in t2_t3_diff_MO) {
       col_mean_t2 = mean(sub_tab_filt[c("SWT2R1", "SWT2R2", "SWT2R3"), col])
       mean_df_t2 <- rbind(mean_df_t2, data.frame(col = col, mean = col_mean_t2))
       col_mean_t3 = mean(sub_tab_filt[c("SWT3R1", "SWT3R2", "SWT3R3"), col])
...
for (col in t3_t5_diff_MO) {
       col_mean_t3 = mean(sub_tab_filt[c("SWT3R1", "SWT3R2", "SWT3R3"), col])
       mean_df_t3 <- rbind(mean_df_t3, data.frame(col = col, mean = col_mean_t3))
       col_mean_t5 = mean(sub_tab_filt[c("SWT5R2", "SWT5R3"), col])
...
getwd()
library(writexl)
write_xlsx(final_df_t01, "table_T0vsT1.xlsx")
write_xlsx(final_df_t12, "table_T1vsT2.xlsx")
write_xlsx(final_df_t23, "table_T2vsT3.xlsx")
write_xlsx(final_df_t35, "table_T3vsT5.xlsx")
```
...
metabolite <- read_excel("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/metabolomica.xlsx") # on the rows there are chemical compounds and on the columns there are samples, how to compute the correlation heatmap between microorganisms and chemical compounds. 
...
```{r pressure, echo=FALSE, warning=FALSE}
sub_metabolite_w <- t_metabolite_new[c("SWT0", "SWT1", "SWT2", "SWT3", "SWT5"),]
```
...
rownames(t_metabolite_new) <- c("SWT0", "SWT1", "SWT2", "SWT3", "SWT5")
```
...
write_xlsx(t_metabolite_new, "/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/8_Corr_Pathways_Metabolites/metabolite_table_modified.xlsx")
```

```{r pressure, echo=FALSE, warning=FALSE}
# take from the subw table only that microorganisms
filt_tab_lef <- filt_tab[rownames(filt_tab) %in% c("SWT0R1", "SWT0R2", "SWT0R3", "SWT1R1", "SWT1R2", "SWT1R3", "SWT2R1", "SWT2R2", "SWT2R3", "SWT3R1", "SWT3R2", "SWT3R3", "SWT5R2", "SWT5R3"),]
...
group_ids <- c("SWT0", "SWT0", "SWT0", "SWT1", "SWT1", "SWT1", "SWT2", "SWT2", "SWT2", "SWT3", "SWT3", "SWT3", "SWT5", "SWT5")
...
```{r pressure, echo=FALSE, warning=FALSE}
library(RColorBrewer)
setwd("/Users/micheladellalma/Library/CloudStorage/OneDrive-UniversitàdegliStudidiMilano/michela_0/Savorgnano/5_Corr_MO_Metabolomics/")
getwd()
...
write.table(results_df, "spearman_correlation.txt", sep = "\t", quote = FALSE, row.names = TRUE)
...
# save into a table
library(writexl)
write_xlsx(significant_df, "MO_corr_metab_eukarya.xlsx")
...
MO_sign_unique <- sub_species
MO_sign_unique <- as.data.frame(MO_sign_unique)
write_xlsx(MO_sign_unique, "MO_bact_DA.xlsx")
```
```{r pressure, echo=FALSE, warning=FALSE}
otu_avg <- tab_avg[,colnames(tab_avg) %in% MO_sign_unique]
dim(otu_avg)
...
# save into a table
library(writexl)
write_xlsx(significant_df, "MO_cor_metab_bact.xlsx")
corr_table <- significant_df
```
```{r pressure, echo=FALSE, warning=FALSE}
library(igraph)
