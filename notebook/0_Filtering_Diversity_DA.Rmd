...
setwd("PATH/WHERE/YOU/HAVE/THE/ABUNDANCE/TABLE")
getwd()
```
...
tab <-read_excel("Abundance_Table.xlsx", col_names=TRUE) # table containing as first column the MO and the others the different samples
...
rows_to_keep <- c("T0R1", "T0R2", "T0R3", 
                ...
                "T2R1", "T2R2", "T2R3",
                "T3R1", "T3R2", "T3R3",
                "T5R2", "T5R3") # subset the total table
...
tab_taxa <- read_excel("taxonomy_bracken.xlsx")
...
png("Saccharomycetac_abundance.png", width = 2000, height = 1500, res = 300)  # 300 dpi is print-quality
...
png("Saccharomyces_abundance.png", width = 2000, height = 1500, res = 300)  # 300 dpi is print-quality
dev.off()
plot(abund_sacch)
```
...
final_taxa <- read_excel("taxonomy_bracken.xlsx")
...
png("Yeast_non_saccharomyces.png", width = 1200, height = 900, res = 200)  # 300 dpi is print-quality
plot(abund_yeast)
dev.off()
plot(abund_yeast)
```
...
# 10. Save plot
png("Yeast_non_saccharomyces_genus_wine.png",
...
# 10. Save plot
png("Yeast_non_saccharomyces_genus_no_oeno.png",
    width = 1200, height = 900, res = 200)
plot(abund_yeast_genus)
dev.off()
...
png("nmds_nobact.png", width = 2000, height = 1500, res = 300)  # 300 dpi is print-quality
ordiplot(nmds, type="n") #Ordination plot function especially for congested plots
orditorp(nmds, display="species", col="red", air=0.01) #The function adds text or points to ordination plots
orditorp(nmds, display="sites", cex=1.25, air=0.01)
dev.off()
...
png("nmds_no_bact_top15.png", width = 2800, height = 1200, res = 200)
ordiplot(nmds, type = "n")
# Add sample points (black)
orditorp(nmds, display = "sites", cex = 1.25, air = 0.01)
# Add only top 15 species (in red)
...
ggplot(nmds_scores, aes(x = NMDS1, y = NMDS2, color = timepoint)) +
  ...
  labs(title = "NMDS of Wine Samples (Bray-Curtis, Hellinger-transformed)",
       x = "NMDS1", y = "NMDS2", color = "Timepoint")
png("nmds_no_bact_top3.png", width = 2800, height = 1200, res = 200)
ordiplot(nmds, type = "n")
# Add sample points (black)
orditorp(nmds, display = "sites", cex = 1.25, air = 0.01)
# Add only top 15 species (in red)
...
png("nmds_bact.png", width = 2000, height = 1500, res = 300)  # 300 dpi is print-quality
ordiplot(nmds, type="n") #Ordination plot function especially for congested plots
orditorp(nmds, display="species", col="red", air=0.01) #The function adds text or points to ordination plots
orditorp(nmds, display="sites", cex=1.25, air=0.01)
dev.off()
...
# subset to sample T0 and T1
# take from the dataframe only the T0 and T1
filt_tab_lef <- filt_tab[rownames(filt_tab) %in% c("T0R1", "T0R2", "T0R3", 
                                                   "T1R1", "T1R2", "T1R3"),]
...
```{r pressure, echo=FALSE}
setwd(".../4_DA_pairwise_w_t/maaslin2")
getwd()
...
## T0 vs T1 ##
```{r pressure, echo=FALSE}
setwd("....../4_DA_pairwise_w_t/maaslin2/T0\ vs\ T1")
...
## T1 vs T2 maaslin 2 ##
```{r pressure, echo=FALSE}
setwd(".../4_DA_pairwise_w_t/maaslin2/T1 vs T2/")
...
## T2 vs T3 maaslin 2 ##
```{r pressure, echo=FALSE}
setwd(".../4_DA_pairwise_w_t/maaslin2/T2 vs T3/")
...
# venn diagram
library(readxl)
lefse <- read_excel(".../4_DA_pairwise_w_t/tab_LEfSe_T0_vs_T1.xlsx")
deseq <- read_csv(".../4_DA_pairwise_w_t/deseqt0vst1.csv")
maaslin <- read.delim(".../4_DA_pairwise_w_t/maaslin2/T0 vs T1/all_results.tsv")
...
```{r pressure, echo=FALSE, warning=FALSE}
library(writexl)
setwd(".../4_DA_pairwise_w_t/ENRICHED/t0vst1/")
getwd()
...
library(readxl)
library(readr)
lefse <- read_excel(".../4_DA_pairwise_w_t/tab_LEfSe_T1_vs_T2.xlsx")
deseq <- read_csv(".../4_DA_pairwise_w_t/deseqt1vst2.csv")
maaslin <- read.delim(".../4_DA_pairwise_w_t/maaslin2/T1 vs T2/all_results.tsv")
...
```{r pressure, echo=FALSE, warning=FALSE}
library(writexl)
setwd(".../4_DA_pairwise_w_t/ENRICHED/t1vst2/")
getwd()
...
library(readxl)
library(readr)
lefse <- read_excel(".../4_DA_pairwise_w_t/tab_LEfSe_T2_vs_T3.xlsx")
deseq <- read_csv(".../4_DA_pairwise_w_t/deseqt2vst3.csv")
maaslin <- read.delim("..../4_DA_pairwise_w_t/maaslin2/T2 vs T3/all_results.tsv")
...
```{r pressure, echo=FALSE, warning=FALSE}
library(writexl)
setwd(".../4_DA_pairwise_w_t/ENRICHED/t2vst3/")
getwd()
...
deseq <- read_csv(".../4_DA_pairwise_w_t/deseqt3vst5.csv")
maaslin <- read.delim(".../4_DA_pairwise_w_t/maaslin2/T3 vs T5/all_results.tsv")
...
```{r pressure, echo=FALSE, warning=FALSE}
library(writexl)
setwd(".../4_DA_pairwise_w_t/ENRICHED/t3vst5/")
getwd()
...
```{r pressure, echo=FALSE, warning=FALSE}
setwd(".../4_DA_pairwise_w_t/results_MO_significant")
getwd()
...
metabolite <- read_excel("metabolomica.xlsx") # on the rows there are chemical compounds and on the columns there are samples, how to compute the correlation heatmap between microorganisms and chemical compounds. 
...
write_xlsx(t_metabolite_new, ".../8_Corr_Pathways_Metabolites/metabolite_table_modified.xlsx")
```
